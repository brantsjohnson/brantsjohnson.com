...
// This loads the facts about Brant (the "knowledge base") from the data
// file and turns them into plain text the AI can read. It is the same
// retrieval/knowledge logic the original BrantChat app uses, kept in one
// shared place so the in-site chat has the exact same "brain". Updating
// Brant's info is just editing data/brant-knowledge.json.
//
// This file imports "server-only" so it can never end up in the browser
// bundle: the private knowledge base stays on the server, always.
// No AI model is called from this file.
// ============================================

import "server-only";
import brantKnowledge from "@/data/brant-knowledge.json";

// THIS SECTION DOES: describe the shape of the facts file so the rest of the
// code gets type help. Optional sections may or may not be present.
...
// about Brant, organized into clear ALL-CAPS sections. This is the only source
// the chatbot may use, and the final line reminds it to stay grounded.
...
KEY EXPERIENCE:
- Principal Product Manager at Crew Finance (Dec 2022 to Mar 2024): Sole product person with full ownership of all product decisions, saved $110K+ by internalizing design work, increased user engagement 20%
- Product Management Analyst at Divvy Pay/Bill.com (May 2022 to Sept 2022): Implemented Pendo analytics, improved data accuracy to 93%
- Founder & Product Manager at BackLocal (Sept 2021 to Sept 2022): Full ownership of product direction as founding team member, led product strategy from concept to revenue-generating launch
- Marketing Analyst at FranklinCovey (Jan 2021 to Dec 2022): Generated 85K+ leads, 4x more than peers
- Account Executive at Utah Business (Oct 2024 to Present): Full-time sales role, consults on marketing strategy
...
${(() => {
  ...
  if (typeof fav.artist === "string") {
    ...
    const songs =
      fav.artist.favoriteSongs && fav.artist.favoriteSongs.length
